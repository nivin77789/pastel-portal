          ...
          <div className="xl:col-span-3 flex flex-col gap-4 min-h-0">
            {aiBannerEnabled && (
              <div className="flex-shrink-0 animate-in fade-in slide-in-from-top-4 duration-500">
                <Link to="/chat" className="group relative block w-full bg-gradient-to-br from-indigo-600 via-blue-600 to-violet-600 rounded-2xl p-4 md:p-5 shadow-xl shadow-blue-500/20 overflow-hidden transition-all duration-500 hover:shadow-blue-500/40 hover:-translate-y-1">
          ...
          <div className="xl:col-span-1 h-full min-h-0 animate-in fade-in slide-in-from-right-8 duration-700 delay-200 flex flex-col gap-4">
            ...
            {userRole === "admin" && (
              <div className="grid grid-cols-2 gap-3 flex-shrink-0 animate-in fade-in slide-in-from-right-4 duration-700 delay-300">
                <Link
                  to="/staffes"
                  className="group relative flex flex-col items-center justify-center gap-2 p-3 h-[110px] rounded-2xl bg-gradient-to-br from-white/10 via-white/5 to-transparent dark:from-white/5 dark:via-white/5 dark:to-transparent backdrop-blur-md border border-white/20 hover:border-indigo-500/50 transition-all duration-300 shadow-sm hover:shadow-lg hover:shadow-indigo-500/20 hover:-translate-y-1 overflow-hidden"
                >
                  <div className="absolute inset-0 bg-indigo-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 blur-xl" />

                  <div className="relative p-2.5 rounded-xl bg-indigo-500/10 group-hover:bg-indigo-500/20 transition-all duration-300 group-hover:scale-110">
                    <Users className="w-5 h-5 text-indigo-500 dark:text-indigo-400" />
                  </div>
                  <span className="relative text-xs font-bold text-slate-700 dark:text-slate-200 text-center tracking-wide group-hover:text-indigo-500 dark:group-hover:text-indigo-400 transition-colors">
                    Manage Staff
                  </span>
                </Link>
                ...
                <button
                  ...
                  className={`group relative flex flex-col items-center justify-center gap-2 p-3 h-[110px] rounded-2xl backdrop-blur-md border transition-all duration-300 shadow-sm hover:shadow-lg hover:-translate-y-1 overflow-hidden ${isManaging
                    ...
                    : "bg-gradient-to-br from-white/10 via-white/5 to-transparent dark:from-white/5 dark:via-white/5 dark:to-transparent border-white/20 hover:border-blue-500/50 hover:shadow-blue-500/20"
                    }`}
                >
                  <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 blur-xl ${isManaging ? "bg-white/10" : "bg-blue-500/10"}`} />

                  <div className={`relative p-2.5 rounded-xl transition-all duration-300 group-hover:scale-110 ${isManaging ? "bg-white/20" : "bg-blue-500/10 group-hover:bg-blue-500/20"
                    }`}>
                    <Settings className={`w-5 h-5 ${isManaging ? "text-white animate-spin-slow" : "text-blue-500 dark:text-blue-400"}`} />
                  </div>
                  <span className={`relative text-xs font-bold text-center tracking-wide transition-colors ${isManaging ? "text-white" : "text-slate-700 dark:text-slate-200 group-hover:text-blue-500 dark:group-hover:text-blue-400"
                    }`}>
